...
${CADDY_PROXY_DOMAIN}:${IMMICH_HTTPS_PORT} {
	reverse_proxy ${SERVER_IP}:${IMMICH_HTTP_PORT}
  # immich
}
...
PublishPort=${IMMICH_HTTPS_PORT}:${IMMICH_HTTPS_PORT}
...
function setEnvVars() {
  ...
  IMMICH_HTTPS_PORT="$(yq -r '.IMMICH.HTTPS_PORT' ${CONFIG_YAML})"
  IMMICH_HTTP_PORT="$(yq -r '.IMMICH.HTTP_PORT' ${CONFIG_YAML})"
